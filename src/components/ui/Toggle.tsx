...
export function Toggle({ checked, onChange, label, disabled }: ToggleProps) {
  ...
  //   disabled  → bg-black/10 dark:bg-white/15 (light, transparent — matches old unchecked look)
  ...
  return (
    <label className={`flex items-center gap-2 select-none ${disabled ? "cursor-not-allowed" : "cursor-pointer"}`}>
      ...
      >
        <span
          className={`inline-block h-3.5 w-3.5 rounded-full bg-brand-canvas shadow transition-transform ${
